...
import EditPendingRequest from '@/app/(dashboard)/(home)/accounts-corporate-sme/(Personnel)/[id]/(company profile)/edit-pending-request'
          ...
          <TooltipTrigger asChild={true}>
            <div>
              <Button className="w-full gap-2 md:max-w-xs" onClick={handleClick}>
                {isBeingEdited ? <Pencil/> : <Pencil/>}<span> Edit Company Details</span>
              </Button>
            </div>
          </TooltipTrigger>
          {isBeingEdited && (
